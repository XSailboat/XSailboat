...
public class CommandLine implements Serializable
{
    ...
    public String checkOptionValue(String aOpt)
    {
    	 String[] values = getOptionValues(aOpt);
    	 if(values == null)
    	 {
    		 Option opt = resolveOption(aOpt) ;
    		 if(opt == null)
    			 throw new IllegalArgumentException("不支持参数选项 "+aOpt) ;
    		 else
    			 throw new IllegalArgumentException(XString.splice("未设置参数 " , aOpt , "  " 
    					 , opt.getDescription()))  ;
    	 }
         return values[0];
    }
    
    public int checkOptionValue_Int(String aOpt)
    {
    	return Integer.parseInt(checkOptionValue(aOpt)) ;
    }
