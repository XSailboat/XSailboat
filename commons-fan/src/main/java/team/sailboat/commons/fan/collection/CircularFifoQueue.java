...
/**
 * 来自Apache Commons Collections 4.1 <br>
 * 固定长度的先入先出的循环队列，非线程安全的
...
public class CircularFifoQueue<E> extends AbstractCollection<E>
    implements Queue<E>, ICircularBoundedQueue<E>, Serializable {
    ...
    {
		final int sz = size();
		if (aOffset < 0 || aOffset >= sz)
		{
			throw new NoSuchElementException(
					String.format("The specified index (%1$d) is outside the available range [0, %2$d)",
							Integer.valueOf(aOffset),
							Integer.valueOf(sz)));
		}
		Assert.isTrue(aLen>=0 , String.format("获取元素的个数必须大于等于0,不能为%1$d", aLen)) ;
    	aLen = Math.min(sz-aOffset , aLen) ;
		E[] array = (E[]) Array.newInstance(aComponentType, aLen);
		int i = 0 ;
        while(i<aLen)
        {
        	array[i] = elements[(start+aOffset+i)%maxElements] ;
        	i++ ;
        }
        return array ;
    }
    ...
    @SuppressWarnings("unchecked")
	@Override
	public E[] poll(int aLen , Class<?> aComponentType)
    {
    	Assert.isTrue(aLen>=0) ;
    	aLen = Math.min(aLen, size()) ; 
    	E[] array = (E[]) Array.newInstance(aComponentType, aLen);
		int i = 0;
    	while(!isEmpty() && i<aLen)
    		array[i++] = remove() ;
    	return array ;
    }
