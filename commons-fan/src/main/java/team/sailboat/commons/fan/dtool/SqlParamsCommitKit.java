...
public class SqlParamsCommitKit implements ICommitKit
{
	...
	public SqlParamsCommitKit(String aSql , int...aColTypes)
	{
		mSql = aSql ;
	...
	public SqlParamsCommitKit(String aSql , String...aColTypes)
	{
	 ...
	 */
	public SqlParamsCommitKit(String aSql , int[] aAddiArgIndex , String...aColTypes)
	{
		mSql = aSql ;
		mColAmount = aColTypes.length ;
...
//					Debug.clockOff("提交 %d 条记录", mCommitSize);
	...
	public void commitImmediately(boolean aAsync) throws SQLException
	{
		if(mCacheSize>0)
		{
			...
			_commit(aAsync , XC.cloneArray(mLastRow));
		}
	}
	/**
	 * 提交缓存着还没提交的数据，并且释放资源
	...
	public int finish() throws SQLException
	{
		...
		{
			mLsnAssist.notifyLsns(new XEvent(new CommitBrief(mTotalCommitAmount , mTotalCommitAmount+mCommitSize , mLastRow), 0));
			_commit(false , XC.cloneArray(mLastRow));
		}
		disposeResource();
		return mCacheSize ;
	}
